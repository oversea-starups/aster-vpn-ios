...
- Home shows the circular power control, the Pro value/upgrade surface, and the current region in that order. Putting the conversion reason immediately after the connection outcome keeps the primary task clear while leaving location switching one tap away. Account is the single destination for subscription, restore and legal actions. `SubscriptionTier` separates capability tiers from billing cadence; only verified StoreKit products mapped in `AppConfiguration` are surfaced (currently monthly/yearly both map to Pro).
