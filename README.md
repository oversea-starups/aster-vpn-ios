...
SwiftUI Home
  -> ConnectionViewModel
  -> NodeCatalogStore
  -> bundled node_catalog.json -> App Group last-known-good
  -> Locations -> selected tunnel_config.json
  -> SubscriptionStore / StoreKit 2
  -> Firebase Analytics (minimal funnel events)
...
`TunnelConfiguration` schema v2 支持 VLESS/VMess、TCP/WS/gRPC、TLS/Reality/uTLS 并兼容 schema v1。首发从 App bundle 内置目录初始化，并保留公开 HTTPS 的大小/协议/安全校验与 App Group 原子 last-known-good cache 作为后续更新路径；失败不会覆盖已验证线路。仓库不保存生产 feed URL，且不得把个人/master subscription token 放进 Info.plist。

Release 必须注入公开 Privacy URL；首发线路来自审核后的 App bundle，`ASTER_NODE_SUBSCRIPTION_URL` 仅作为未来可撤销更新入口。App Store target 为 StoreKit-only，不包含广告 SDK、UMP、rewarded balance、广告标识或追踪。

历史 AdMob SSV verifier 保留在 [Backend/AdMobSSV/README.md](Backend/AdMobSSV/README.md)，仅供归档/实验复盘；它不属于当前 App Store target，也不是首发发布依赖。
