...
- **Release decision:** **Not release-ready.** StoreKit-only 已解决 AdMob 与 Apple VPN 审核规则的产品冲突；双语隐私政策/服务条款已发布，App Store Connect App Privacy 标签已发布并与当前 Firebase Analytics、无广告、无账号实现对齐；生产节点源、签名、StoreKit sandbox、公共 fd bridge 真机回归和完整设备矩阵仍未完成。
...
- **Current VPN correction (supersedes the earlier installed-package state):** 用户点击连接后的设备日志已把失败定位为三层 Libbox 兼容问题：空 override options 导致 Go panic，旧 archive 未启用 uTLS，以及精简构建移除了 `with_clash_api`，导致 `startOrReloadService` 创建内部 clash-server 失败。当前已用 pinned upstream source 重建 `with_utls,with_clash_api,ios,with_low_memory` Libbox（Clash API 仅作为内部启动依赖，不对外暴露），加入 config preflight 和非空 options，并将 Provider 启动工作移出 XPC 主线程；iPhoneOS App build 与 nested codesign 需在新 framework 上重新验证。VPN 握手、DNS/HTTPS/出口与连接稳定性仍未验证，不能把安装成功等同于 device runtime verified。
- **Current connection UX/reliability correction (2026-09-01):** 连接图标已改为静态状态图标，不再使用无限循环旋转；连接尝试增加 25 秒超时并在失败后回到可重试状态。连接前会幂等修复 App Group 中缺失/中断的 `tunnel_config.json`，首次无选中配置时优先选择内置 AnyTLS 线路（保留 VMess/VLESS 供手动选择），并只复用匹配当前 Packet Tunnel bundle ID 的系统 VPN manager。源码与 arm64 device build 均通过；17:37 已重新安装并启动到 Thomson’s iPhone，VPN runtime（权限、握手、DNS/HTTPS/出口）仍待用户点击后回归。
...
- **PacketTunnel Debug crash correction (2026-09-02):** Thomson iPhone crash reports showed the Debug PacketTunnel extension injecting LLVM coverage and attempting to create `default.profraw` inside its read-only bundle immediately after the default-interface monitor started; the process then exited with SIGABRT and iOS reported `Plugin failed`. PacketTunnel Debug now disables code coverage/testability instrumentation and Libbox debug probes; the interface listener remains enabled and runs on a system utility queue. The updated arm64 package builds and installs successfully; a post-fix tap/traffic run is still required before marking device runtime verified.
...
- Home and Account do not expose ads or privacy-choice controls. A new user receives one ten-minute first-connection experience; the claim is protected by Keychain and is not a daily balance.
...
- Locations 的 VIP Tab 进入后直接加载并展示 StoreKit 返回的套餐名称、描述和本地化价格；用户选择套餐后可直接发起 Apple 购买，不再需要额外的“查看套餐”中间按钮。StoreKit 原生试用仅对符合资格的用户显示。
...
- Bundled Libbox 固定到 sing-box commit `650ef881c8fb216259e4ebcfbd74234554c39612`，device binary SHA-256 `92e997f1b5740c5e7c34d5fb272b167668aa7da714371ff30eccbd22cbc74313`，simulator binary SHA-256 `c0bd23646fa71e0acd20ad657468fcbc41eff19eaeb8d8707664fb9c218f861c`，tags 为 `with_utls,with_clash_api,ios,with_low_memory`。构建 provenance 已记录，GPL 分发合规仍未关闭。
...
| Real VPN connection | Corrected `with_utls,with_clash_api,ios,with_low_memory` package installed on Thomson’s iPhone; device log shows `Libbox command server started`, `Tunnel file descriptor delivered`, and `status changed to connected` / `NESMVPNSessionStateRunning` with no prior Clash API error | Device bootstrap/connection state verified; exit-IP, DNS leak and multi-line traffic matrix still pending |
